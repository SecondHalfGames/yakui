...
fn my_custom_paint(
    ...
    buffers: &mut yakui_wgpu::Buffers,
    state: &mut Yakui,
    ...
    yakui_wgpu.update_textures(paint);
    ...
    let mut globals = paint.globals.borrow_mut();
    let renderer = globals.get_mut(MyCustomRenderer::new);
    ...
    for (clip, call) in layers.iter().flat_map(|layer| &layer.calls) {
        match call {
            PaintCall::Internal(call) => {
                draw_calls.push(yakui_wgpu.build_draw_call(buffers, *clip, call));
            }
            PaintCall::User(id) => {
                let min = clip.pos();
                let max = clip.max();
    ...
    let vertices = buffers
        .vertices
        .upload(&yakui_wgpu.device, &yakui_wgpu.queue);
    let indices = buffers
        .indices
        .upload(&yakui_wgpu.device, &yakui_wgpu.queue);
    // --- yakui ---

    {
        // --- yakui ---
        let main_pipeline =
            yakui_wgpu::main_pipeline(&mut yakui_wgpu.main_pipeline, &yakui_wgpu.device, &surface);
        let text_pipeline =
            yakui_wgpu::text_pipeline(&mut yakui_wgpu.text_pipeline, &yakui_wgpu.device, &surface);
