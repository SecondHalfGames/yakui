...
impl InnerTextState {
    fn fonts_changed(&mut self, paint: &mut PaintDom) {
        self.atlas.mask_atlas.clear(paint);
        self.atlas.color_atlas.clear(paint);
...
impl TextGlobalState {
    ...
    pub fn fonts_changed(&mut self, paint: &mut PaintDom) {
        self.inner.borrow_mut().fonts_changed(paint)
    }
