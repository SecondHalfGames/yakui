...
impl Widget for TextBoxWidget {
    ...
    fn update(&mut self, props: Self::Props<'_>) -> Self::Response {
        ...
        self.text_changed
            .set(text_changed_by_caller || style_changed || text_changed_by_cosmic);
        ...
        fonts.with_system(|font_system| {
            ...
            if let Some(editor) = self.cosmic_editor.borrow_mut().as_mut() {
                ...
                if text_changed_by_caller || style_changed || text_changed_by_cosmic {
                    editor.with_buffer_mut(|buffer| {
    ...
    fn layout(&self, ctx: LayoutContext<'_>, constraints: Constraints) -> Vec2 {
        ...
        let max_width = if max_width.is_normal() {
            let max_width = max_width - self.props.padding.offset().x * 2.0;
            ...
            let max_width = self.props.min_width - self.props.padding.offset().x * 2.0;
            if max_width.is_normal() {
                Some((max_width * ctx.layout.scale_factor()).ceil())
            } else {
                None
        ...
        let max_height = if max_height.is_normal() {
            let max_height = max_height - self.props.padding.offset().y * 2.0;
            ...
            if let Some(editor) = self.cosmic_editor.borrow_mut().as_mut() {
                ...
                if text_changed || size_changed {
                    ...
                    editor.with_buffer_mut(|buffer| {
                        ...
                        buffer.set_size(font_system, Some(self.max_text_width.get()), max_height);
                    });
                }
            }
        });
    ...
    fn event(&mut self, ctx: EventContext<'_>, event: &WidgetEvent) -> EventResponse {
        ...
        match event {
            ...
            WidgetEvent::MouseScroll {
                ...
                if let Some(editor) = self.cosmic_editor.get_mut() {
                    fonts.with_system(|font_system| {
                        editor.action(font_system, cosmic_text::Action::Scroll { pixels: delta.y });
