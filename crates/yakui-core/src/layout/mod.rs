...
impl LayoutDom {
    ...
    fn resolve_clipping(&mut self, dom: &Dom) {
        ...
        while let Some((id, parent_clip, parent_rect, offset)) = queue.pop_front() {
            ...
            let layout_rect = logic.resolve_layout(ClipResolutionArgs {
                parent_clip,
                parent_rect,
                layout_rect: layout_node.rect,
                viewport,
                offset,
            });
            let new_offset = layout_rect.pos() - layout_node.rect.pos();
            ...
            queue.extend(
                node.children
                    .iter()
                    .map(|&id| (id, layout_node.clip, layout_node.rect, new_offset)),
            );
        }
    }
}
