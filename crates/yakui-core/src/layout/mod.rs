...
impl LayoutDom {
    ...
    pub fn enable_clipping(&mut self, dom: &Dom) {
        self.set_clip_logic(
            dom,
            ClipLogic::Constrain(AbstractRect::ParentClip, AbstractRect::LayoutRect),
        );
    }

    /// Escapes clipping from the current clipping rect for the currently active widget.
    pub fn escape_clipping(&mut self, dom: &Dom) {
        self.set_clip_logic(dom, ClipLogic::Override(AbstractRect::Viewport));
    }
    ...
    fn resolve_clipping(&mut self, dom: &Dom) {
        ...
        while let Some((id, parent_clip)) = queue.pop_front() {
            ...
            let new_clip = logic.resolve(parent_clip, layout_node.rect, viewport);

            layout_node.clip_rect = new_clip;

            queue.extend(node.children.iter().map(|&id| (id, new_clip)));
        }
    }
}
